Shader "SupGames/PlanarReflection/Diffuse Vertex Lit" {
	Properties{
	...
	SubShader{
		...
		Pass
		{
			...
			v2f vert(appdata v)
			{
				...
				o.diff.xyz = _LightColor0.rgb * dot(normalize(mul(fixed4(v.normal, 0.0h), unity_WorldToObject).xyz), _WorldSpaceLightPos0.xyz);
...
#endif
				color = fixed4((UNITY_LIGHTMODEL_AMBIENT.rgb + i.diff.xyz * LIGHT_ATTENUATION(i)) * color.rgb, 1.0h);
